...
MongoClient.connect(url, function (err, database) {
      ...
      doAuth(clientData, function (data) {
        try {
          ...
          if (id == '' || id == '_' || id == 'undefined_undefined') {
            ...
            //update(id, data);
            //debugger;
            //console.log('enqueue initShift');
            kue.push(function () {
              //console.log('start initShift job');
              return initShift(id, data, socket);
            });
            //kue.start();
          }
        }
          ...
          if (id == '' || id == '_' || id == 'undefined_undefined') {
            ...
            kue.push(function () {
              return new Promise(function (res, rej) {
                res(updateOrder(id, data, socket));
              })
            });
            //kue.start();
          }
      ...
      doAuth(data, function (data) {
        try {
          ...
          if (id == '' || id == '_' || id == 'undefined_undefined') {
            ...
            kue.push(function () {
              return moveOrder(id, data, socket);
            });
            //kue.start();
          }
        }
      ...
      doAuth(data, function (data) {
        try {
          ...
          if (id == '' || id == '_' || id == 'undefined_undefined') {
            ...
            kue.push(function () {
              return syncOfflineOrder(id, data, socket);
            });
            //kue.start();
          }
        }
      ...
      doAuth(data, function (data) {
        try {
          ...
          if (id == '' || id == '_' || id == 'undefined_undefined') {
            ...
            //console.log('enqueue completeOrder');
            kue.push(function () {
              //console.log('start completeOrder job');
              return completeOrder(id, data, socket);
            });
            //kue.start();
          }
        }
      ...
      doAuth(data, function (data) {
        try {
          ...
          if (id == '' || id == '_' || id == 'undefined_undefined') {
            ...
            kue.push(function () {
              return completeShift(id, data, socket);
            });
            //kue.start();
          }
        }
      ...
      doAuth(data, function (data) {
        try {
          ...
          if (id == '' || id == '_' || id == 'undefined_undefined') {
            ...
            kue.push(function () {
              return clearShift(id, data, socket);
            });
            //kue.start();
          }
        }
      ...
      doAuth(data, function (data) {
        try {
          ...
          if (id == '' || id == '_' || id == 'undefined_undefined') {
            ...
            completeShiftForNewVersion(id, data, socket);
          }
        }
        catch (ex) {
          data.ipAddress = socket.handshake.address;
      ...
      doAuth(data, function (data) {
        try {
          ...
          if (id == '' || id == '_' || id == 'undefined_undefined') {
            ...
            kue.push(function () {
              return printHelper(id, data, socket);
            });
            //kue.start();
          }
        }
  ...
  var initShift = function (id, data, socket) {
    var prom = new Promise(function (res, rej) {
          ...
          if (docs == null || docs == undefined || docs == [] || docs.length == 0) {
            ...
            if (data.tables && data.tables.length > 0 && shiftIdReq == shiftIdCur) {
              ...
              for (var i = 0; i < data.tables.length; i++) {
                ...
                for (var j = 0; j < data.tables[i].tableOrder.length; j++) {
                  ...
                  if (t) {
                    ...
                    //- Có thì đơn hàng Client gửi lên không hợp lệ (Trường hợp đăng nhập cùng 1 tài khoản trên 2 thiết bị, thoát 1 thiết bị nhưng vẫn còn lưu ở DB Local và thao tác ở thiết bị còn lại)
              ...
              collection.update({ companyId: data.companyId, storeId: data.storeId }, { $set: { tables: docs[0].tables } }, { w: 1, multi: true }, function (err, result) {
                ...
                data.msg = msg;
                //data.orderHisLog = Array.prototype.concat.apply([], docHis[0].tables.map(function (tb) {
                //  return tb.tableOrder.map(function (order) {
                //    return order.saleOrder.saleOrderUuid;
                //  })
                //}));
                //Giai đoạn 2: Trả data phù hợp về cho Client
                //console.log('done initShit job - do next job');
                io.to(id).emit('initShift', data);
                res(true);
              });
            }
      ...
      setTimeout(function () {
        getPromiseState(prom, function (state) {
          if (state === "pending") {
            //console.log('auto done initShit job - do next job');
            logDebug('auto done initShit job - do next job');
            res(true);
          }
        })
      }, 10000);
  ...
  var updateOrder = function (id, data, socket) {
    var prom = new Promise(function (res, rej) {
      var shiftIdCur = null;
      var shiftIdReq = data.shiftId;
      var collection = db.collection('tableOrder');
      ...
      setTimeout(function () {
        getPromiseState(prom, function (state) {
          if (state === "pending") {
            logDebug('auto done updateOrder');
            res(true);
          }
        })
      }, 5000);
  ...
  var syncOfflineOrder = function (id, data, socket) {
    return initShift(id, data, socket);
  }

  //Hàm xử lý khi hoàn tất đơn hàng hoặc xóa trống đơn hàng đã báo bếp.
  var completeOrder = function (id, data, socket) {
    //debugger;
    //console.log('runnin completeOrder job');
    var prom = new Promise(function (res, rej) {
      var shiftIdReq = data.shiftId;
      var shiftIdCur;
                  ...
                  if (completed.length > 0) {
                    history.find({ companyId: data.companyId, storeId: data.storeId, shiftId: shiftIdCur }).sort({ startDate: -1 }).toArray(function (err, docs) {
                      ...
                      history.update({
                        ...
                        if (err) logDebug('Error:' + err);
                        //console.log('done completeOrder job - do next job');
                        res(true);
                      });
                    });
                  }
      ...
      setTimeout(function () {
        getPromiseState(prom, function (state) {
          if (state === "pending") {
            logDebug('auto done completedOrder');
            res(true);
          }
        })
      }, 5000);
  ...
  var moveOrder = function (id, data, socket) {
    var prom = new Promise(function (res, rej) {
      var shiftIdReq = data.shiftId;
      var shiftIdCur;
      var tableOrder = db.collection('tableOrder');
  ...
  var completeShift = function (id, data, socket) {
    var prom = new Promise(function (res, rej) {
      var shiftIdReq = data.shiftId;
      var shiftIdCur;
      var tableOrder = db.collection('tableOrder');
      ...
      setTimeout(function () {
        getPromiseState(prom, function (state) {
          if (state === "pending") {
            logDebug('auto done completeShift');
            res(true);
          }
        })
      }, 7000);
  ...
  var clearShift = function (id, data, socket) {
    var prom = new Promise(function (res, rej) {
      var tableOrder = db.collection('tableOrder');
      var serverLog = db.collection('serverLog');
      var history = db.collection('tableOrderHistory');
  ...
  var completeShiftForNewVersion = function (id, data, socket) {
    var tableOrder = db.collection('tableOrder');
    var serverLog = db.collection('serverLog');
    var history = db.collection('tableOrderHistory');
    // Find some documents 
